...
def safe_read_csv(path: Path, label: str) -> pd.DataFrame:
    """Read a CSV only if it has content. Return an empty DataFrame instead of crashing."""
    if not path.exists():
        st.info(f"{label} was not created for this run.")
        return pd.DataFrame()
    if path.stat().st_size == 0:
        st.warning(f"{label} is empty. The scraper ran, but no rows were extracted into this file.")
        return pd.DataFrame()
    try:
...
if latest_output_dir and latest_output_dir.exists():
    ...
    sources_df = safe_read_csv(sources_path, "Sources CSV")
    if not sources_df.empty:
        st.markdown("**Sources Found / Attempted**")
        st.dataframe(sources_df, use_container_width=True)
    elif sources_path.exists():
        st.info("No source records were written for this run.")

    comparison_df = safe_read_csv(comparison_path, "Comparison CSV")
    if not comparison_df.empty:
        st.markdown("**Comparison Pivot Ready Rows**")
        st.dataframe(comparison_df, use_container_width=True)
    elif comparison_path.exists():
        st.warning(
            "No comparison-ready compensation rows were extracted. This usually means the district sites did not expose usable tables, "
            "the scraper could not access the relevant documents, or the salary schedules are in PDF/image formats that need manual cleanup. "
            "You can still download the Sources CSV to see what the app attempted."
        )
